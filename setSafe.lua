...
if (not KEYS[keyIndex]) then
    return redis.error_reply("Not all keys were provided to setSafe");
end
...
if (not ARGV[argIndex]) then
    return redis.error_reply("Not all args were provided to setSafe");
end

if (ARGV[argIndex + 1]) then
    return redis.error_reply("Extra args were provided to setSafe");
end
...
redis.call("SET", TARGET_KEY, value, "EX", ttlSec);
